 ...
 * Internal function defined in peripheral.h.
 *
 * osal¿¡ ÅÂ½ºÅ©¸¦ µî·ÏÇØµÎ°í ÇØ´ç ÅÂ½ºÅ©·Î ÀÌº¥Æ®°¡ ¹ß»ý ÇßÀ» °æ¿ì¿¡ Ã³¸®ÇØÁÖ´Â ÇÔ¼ö.
 */
 ...
 * @return  none
 *
 * osal¿¡¼­ ½Ã½ºÅÛ ¸Þ½ÃÁö¸¦ ³¯¸®¸é GAPRole_ProcessEvent¿¡¼­ ½Ã½ºÅÛ ÀÌº¥Æ® ¸Þ½ÃÁö¸¦ ÆÇº°ÇÏ¿© ÀÌ ¸Þ¼Òµå¸¦ ÄÝÇÔ.
 *
 */
static void gapRole_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
...
static void gapRole_ProcessGAPMsg( gapEventHdr_t *pMsg )
{
  ...
  switch ( pMsg->opcode )
  {
    ...
    case GAP_LINK_ESTABLISHED_EVENT: // gap ¸µÅ©°¡ ¿¬°áµÇ¾úÀ» ¶§ ÀÌº¥Æ®?
      {
    ...
    case GAP_LINK_TERMINATED_EVENT: // ¸µÅ©°¡ ²÷°åÀ» ¶§ ÀÌº¥Æ®?
      {
    ...
    case GAP_LINK_PARAM_UPDATE_EVENT: // ¸µÅ©ÀÇ ÆÄ¶ó¹ÌÅÍ°¡ ¾÷µ¥ÀÌÆ® µÆ´ÂÁö È®ÀÎ?
      {
